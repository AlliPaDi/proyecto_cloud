...
- **`networks.internet_access`:** Booleano (default `FALSE`). Cuando es `TRUE`, el módulo Networking genera comandos NAT (`iptables MASQUERADE`) para habilitar salida a Internet desde esa red.
- **`security_rules`:** Tabla de reglas de micro-segmentación por OpenFlow. Cada regla define una política de tráfico (ALLOW/DENY) entre dos VMs (`src_vm_id`, `dst_vm_id`) con filtro por protocolo (tcp/udp/icmp/any) y rango de puertos. Se aplica vía `ovs-ofctl` en el Br-Slice, no con `iptables`. Usa `ON DELETE CASCADE` desde `slices` y `virtual_machines`.
...
- Usar `ON DELETE CASCADE` en tablas dependientes de `slices` (incluye `virtual_machines`, `networks`, `tasks`, `security_rules`) para asegurar una limpieza completa de la base de datos al eliminar una topología.
